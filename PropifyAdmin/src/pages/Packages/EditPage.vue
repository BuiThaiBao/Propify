<template>
  <div class="form-page">
    ...
    <div class="form-page-header">
      <button class="back-btn" @click="router.push({ name: 'Packages' })" id="back-btn-edit">
      ...
      <div class="form-page-title-wrap">
        <h1 class="form-page-title">Chỉnh sửa gói tin</h1>
        <p class="form-page-subtitle">Cập nhật cấu hình gói dịch vụ</p>
      </div>

      <!-- Active toggle at top right -->
      <div v-if="packageLoaded" class="active-toggle-wrap">
        <label class="active-toggle" :class="form.is_active ? 'active-toggle--on' : 'active-toggle--off'">
          <input type="checkbox" v-model="form.is_active" id="pkg-is-active" class="visually-hidden" />
          <span class="toggle-track">
            <span class="toggle-thumb"></span>
          </span>
          <span>{{ form.is_active ? 'Đang hoạt động' : 'Đã khóa' }}</span>
        </label>
      </div>
    </div>

    <!-- Loading state -->
    <div v-if="loading && !packageLoaded" class="state-box">
      <div class="spinner"></div>
      <span>Đang tải thông tin gói tin...</span>
    </div>

    <!-- Fetch error -->
    <div v-if="fetchError" class="alert alert--error" role="alert">
    ...
    <form v-if="packageLoaded" @submit.prevent="handleSubmit" class="form-body" novalidate>
              ...
              <div class="duration-panel">
                ...
                <div class="duration-add-row">
                  <input
                    ...
                    id="new-duration-input-edit"
                  />
                  <button type="button" class="btn-add-duration" @click="addDuration" id="add-duration-btn-edit">
                    + Thêm
                  </button>
                </div>
                  ...
                  <table class="duration-table">
                    ...
                    <tbody>
                      <tr v-for="days in visibleDurations" :key="days">
                        ...
                        <td>
                          <label :for="`dur-edit-${days}`" class="dur-label">{{ days }} ngày</label>
                        </td>
                        <td class="text-right">
      ...
      <div class="form-actions">
        <button type="button" class="btn-cancel" @click="router.push({ name: 'Packages' })" id="cancel-edit-btn">
          Hủy bỏ
        </button>
        <button type="submit" class="btn-submit" :disabled="loading" id="submit-edit-btn">
...
async function addDuration() {
  ...
  if (!durationOptions.value.some((o) => Number(o.days) === days)) {
    ...
    if (response?.data) durationOptions.value.push(response.data)
  }
...
function validate() {
  ...
  if (form.priority < 1) { errors.priority = 'Độ ưu tiên phải ≥ 1'; isValid = false }
  if (form.multiplier < 1) { errors.multiplier = 'Hệ số điểm phải ≥ 1'; isValid = false }
  if (form.daily_quota < 0) { errors.daily_quota = 'Quota không hợp lệ'; isValid = false }
  if (form.decay_rate < 0 || form.decay_rate > 1) { errors.decay_rate = 'Decay rate phải từ 0 đến 1'; isValid = false }
  if (form.price < 0) { errors.price = 'Giá không thể âm'; isValid = false }
...
.active-toggle--on { border-color: #bbf7d0; background: #f0fdf4; color: #16a34a; }
.active-toggle--off { border-color: #e2e8f0; background: #f8fafc; color: #64748b; }
...
.active-toggle--on .toggle-track { background: #22c55e; }
...
.active-toggle--on .toggle-thumb { transform: translateX(16px); }
