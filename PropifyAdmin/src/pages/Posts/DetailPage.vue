...
const formatDateTime = (value) => formatAdminDateTime(value).replace('--', EMPTY_LABEL)
const displayValue = (value) => displayAdminValue(value, EMPTY_LABEL)
const formatMetric = (value, unit) => formatAdminMetric(value, unit, EMPTY_LABEL)
const sharedPropertyTypeLabel = (value) => formatAdminPropertyType(value).replace('--', EMPTY_LABEL)
const posterTypeLabel = (value) => formatAdminPosterType(value)

function mapStatusLabel(status) {
  return (
    listingStatusOptions.value.find((option) => option.value === status)?.label ||
    status ||
    'Chờ duyệt'
  )
}
...
function adminStatusLabel(status) {
  return (
    adminListingStatusOptions.value.find((option) => option.value === status)?.label ||
...
function movePan(event) {
  ...
  if (
    Math.abs(event.clientX - panState.value.originX) > 3 ||
    Math.abs(event.clientY - panState.value.originY) > 3
  ) {
  ...
  <div class="detail-page">
        ...
        <div>
          ...
          <div class="meta-row">
            ...
            <span
              ...
              {{ formatDateTime(post.published_at || post.submitted_at) }}</span
            >
          </div>
        </div>
      ...
      <section v-if="activeTab === 'info'" class="section-stack">
        ...
        <article class="detail-card">
          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              <span>Loại BĐS</span>
              <strong>{{ sharedPropertyTypeLabel(property.type) }}</strong>
            </div>
            <div class="info-item">
          ...
          <div v-if="isSaleListing" class="unit-price">
            Đơn giá: <strong>{{ unitPrice }}</strong>
          </div>
          <div v-else-if="isRentListing" class="rent-terms">
            <span
              >Thời gian tối thiểu: <strong>{{ rentMinTermText }}</strong></span
            >
            <span
              >Kỳ thanh toán: <strong>{{ rentPaymentText }}</strong></span
            >
            <span
              >Đặt cọc: <strong>{{ rentDepositText }}</strong></span
            >
          </div>
        </article>
        ...
        <article class="detail-card">
          ...
          <div class="info-grid detail-grid">
            <div class="info-item">
              <span>Phòng ngủ</span><strong>{{ displayValue(property.bedrooms) }}</strong>
            </div>
            <div class="info-item">
              <span>Phòng tắm</span><strong>{{ displayValue(property.bathrooms) }}</strong>
            </div>
            <div class="info-item">
              <span>Số tầng</span><strong>{{ displayValue(property.floors) }}</strong>
            </div>
            <div class="info-item">
              <span>Tầng</span><strong>{{ displayValue(property.floor_number) }}</strong>
            </div>
            <div class="info-item">
              <span>Mặt tiền</span><strong>{{ formatMetric(property.facade_width, 'm') }}</strong>
            </div>
            <div class="info-item">
              <span>Chiều sâu</span><strong>{{ formatMetric(property.depth, 'm') }}</strong>
            </div>
            <div class="info-item">
              <span>Hướng nhà</span><strong>{{ directionLabel(property.direction_code) }}</strong>
            </div>
            <div class="info-item">
              <span>Hướng ban công</span
              ><strong>{{ directionLabel(property.balcony_direction_code) }}</strong>
            </div>
            <div class="info-item">
              <span>Nội thất</span><strong>{{ furnitureLabel(property.furniture_status) }}</strong>
            </div>
            <div class="info-item">
              <span>Ban công</span><strong>{{ displayValue(property.balconies) }}</strong>
            </div>
            <div class="info-item">
              <span>Thỏa thuận</span><strong>{{ property.is_negotiable ? 'Có' : 'Không' }}</strong>
            </div>
          </div>
        ...
        <article class="detail-card">
          ...
          <div class="info-grid">
            <div class="info-item">
              <span>Tỉnh/TP</span
              ><strong>{{ displayValue(property.province_name || property.province_code) }}</strong>
            </div>
            <div class="info-item">
              <span>Phường/Xã</span
              ><strong>{{ displayValue(property.ward_name || property.ward_code) }}</strong>
            </div>
            <div class="info-item">
              <span>Đường</span><strong>{{ displayValue(property.street_code) }}</strong>
            </div>
            <div class="info-item wide">
              <span>Địa chỉ chi tiết</span
              ><strong>{{ displayValue(property.address_detail) }}</strong>
            </div>
          </div>
        ...
        <article class="detail-card">
          ...
          <div class="info-grid">
            <div class="info-item">
              <span>Loại</span><strong>{{ posterTypeLabel(property.poster_type) }}</strong>
            </div>
            <div class="info-item">
              <span>Họ tên</span
              ><strong>{{ displayValue(property.contact_name || post.owner?.full_name) }}</strong>
            </div>
            <div class="info-item">
              <span>Số điện thoại</span><strong>{{ displayValue(property.contact_phone) }}</strong>
            </div>
            <div class="info-item">
              <span>Email</span
              ><strong>{{ displayValue(property.contact_email || post.owner?.email) }}</strong>
            </div>
          </div>
        </article>
      </section>
        ...
        <article class="detail-card">
          ...
          <div v-else-if="listingReports.length" class="warning-list">
            <article
              ...
              :key="report.report_group_id || report.id"
              class="warning-card"
            >
              <div class="warning-avatar">
                <img
                  ...
                  :alt="report.reporter.full_name || 'Người dùng'"
                />
              ...
              <div class="warning-body">
                ...
                <p v-if="report.description" class="warning-description">
                  {{ report.description }}
                </p>
        ...
        <article class="detail-card">
          ...
          <div v-if="histories.length" class="history-list">
            <div v-for="history in histories" :key="history.id" class="history-item">
              ...
              <span
                >{{ history.user?.full_name || `User #${history.user_id}` }} ·
    ...
    <div v-if="reasonModal.open" class="modal-backdrop" @click.self="reasonModal.open = false">
      <div class="modal-card">
        ...
        <div
          v-if="
            reasonModal.type === 'verification' ||
            ['REJECTED', 'LOCKED'].includes(reasonModal.status)
          "
    ...
    >
      ...
      <div class="lightbox-content" :class="{ zoomed: lightboxZoomed }">
        <img
          ...
          :style="
            lightboxZoomed ? { transform: `translate(${panState.x}px, ${panState.y}px)` } : null
          "
...
.lightbox-content {
  max-width: min(980px, calc(100vw - 176px));
  max-height: 100%;
  display: grid;
...
.lightbox-content.zoomed {
  width: min(1400px, calc(100vw - 176px));
  justify-items: start;
  align-items: start;
}
