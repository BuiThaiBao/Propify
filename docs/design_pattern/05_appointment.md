...
Một cuộc đặt lịch hẹn xem nhà trải qua rất nhiều trạng thái khác nhau: PENDING (Chờ duyệt) -> APPROVED (Đã duyệt) hoặc CANCELLED (Đã hủy), và APPROVED -> COMPLETED (Đã hoàn thành). Hệ thống có các quy tắc nghiêm ngặt: Chủ nhà chỉ được confirm/reject khi ở PENDING; cả hai bên chỉ được hủy khi ở PENDING hoặc APPROVED và trước giờ hẹn ít nhất 2 tiếng. Nếu không dùng State Pattern, mã nguồn sẽ ngập tràn các câu lệnh `if (status == ...)` lồng nhau và việc thêm một trạng thái mới (như NOSHOW - khách không đến) sẽ đòi hỏi phải tìm kiếm và sửa đổi code rải rác ở hàng chục phương thức của Service.
...
- **Khóa chặt luồng chuyển đổi trạng thái**: Tránh được các lỗi logic khi viết nhiều khối if-else lồng nhau để kiểm soát xem ở trạng thái này có được gọi hành động kia hay không. Các quy tắc chuyển đổi trạng thái được tự động quản lý rõ ràng bên trong từng lớp State cụ thể.
...
Khi thực hiện một hành động như xác nhận hay hủy lịch hẹn, hệ thống cần thực hiện nhiều bước bên cạnh việc đổi trạng thái: lưu log ghi chú lý do hủy, ghi nhận vào database, và phát đi các sự kiện (Events) để thông báo cho người dùng qua WebSocket hoặc Email. Nếu đặt tất cả logic này vào trong lớp State, các lớp State sẽ bị phụ thuộc vào database và infrastructure, vi phạm Single Responsibility Principle (SRP) và khiến code State cực kỳ khó đọc và khó test.
...
- **Cô lập tác vụ thay đổi dữ liệu**: Việc tách biệt logic lưu trữ (`save()`) và phát sự kiện ra khỏi các State Object giúp cho các State Object chỉ tập trung duy nhất vào logic kiểm soát trạng thái. Tầng Command làm nhiệm vụ điều phối cơ sở hạ tầng (Database + Event Dispatcher).
...
Thời gian chờ xác nhận cuộc hẹn (`confirm_deadline`) được tính toán dựa trên các luật khác nhau: Đặt hẹn mặc định (hạn xác nhận là 6 tiếng sau khi đặt) và Đặt hẹn gấp (khoảng cách đặt hẹn và giờ gặp dưới 6 tiếng, hạn xác nhận được rút ngắn để chủ nhà kịp xử lý: `Thời gian gặp - 1 tiếng`, tối thiểu 1 tiếng). Nếu viết logic này bằng các câu lệnh `if-else` trong luồng tạo lịch hẹn chính, mã nguồn sẽ bị rối rắm và khó kiểm soát kiểm thử cho từng trường hợp tính hạn dùng riêng biệt.
